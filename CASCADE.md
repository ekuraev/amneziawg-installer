...
# 1) Обновить список RU-сетей во временный файл; рабочий заменить только при успешной загрузке.
mkdir -p "$AWG_DIR"
if curl -fsS --retry 2 -o "$RU_ZONE.tmp" "$RU_ZONE_URL" && [ -s "$RU_ZONE.tmp" ]; then
    mv -f "$RU_ZONE.tmp" "$RU_ZONE"
else
    rm -f "$RU_ZONE.tmp"
    echo "WARN: не удалось скачать ru.zone, использую прежний список" >&2
fi
# Без списка не продолжаем: пустой ipset отправит ВЕСЬ трафик за границу (деление молча сломается).
[ -s "$RU_ZONE" ] || { echo "ERROR: список RU-сетей пуст и не скачан - прерываюсь, чтобы не сломать деление" >&2; exit 1; }
...
1. Скачивает список российских сетей во временный файл и подменяет рабочий только при успешной загрузке - сорванная закачка не обнулит уже загруженный список.
