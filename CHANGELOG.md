...
**v5.19.2** - аудит командного слоя (вклад @ekuraev): прерванная установка с CLI-параметрами больше не пропускает перегенерацию, смена порта убирает старое UFW-правило, а `manage repair-module` и `manage add` перестали рапортовать успех там, где его нет.
