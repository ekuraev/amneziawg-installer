...
- **Явная настройка изоляции клиентов** (`--isolation=on|off`, интерактивный вопрос при первой установке). Раньше изоляция была неявным побочным эффектом режима маршрутизации: split-режимы изолировали клиентов только потому, что клиент не знал маршрута к соседям, режим `0.0.0.0/0` — не изолировал вовсе, а dual-stack клиенты в split-режимах оставались достижимы друг для друга по IPv6-подсети туннеля. Теперь при включённой изоляции (по умолчанию) сервер честно блокирует трафик между клиентами (`FORWARD awg0→awg0 DROP`, IPv4 и IPv6) во всех режимах; при отключённой — подсеть туннеля добавляется в `AllowedIPs` клиентов, и устройства видят друг друга. Настройка сохраняется в `awgsetup_cfg.init` (ключ `CLIENT_ISOLATION`), меняется переустановкой; существующим клиентам после смены нужен `manage regen --reset-routes` (подсказка печатается). Правила изоляции снимаются PostDown и добираются явной очисткой при переустановке on→off (#178)
